...
afterAll(async () => {
  await mongoose.connection.db.dropDatabase('storage');
  mongoose.disconnect();
});
...
describe('data-storage-api-node', () => {
  ...
  test('PUT deduplication not allowed within repository', async () => {
    await supertest(server)
      .put('/data/cats')
      .send({ name: 'Emmea' })
      .set('Accept', 'application/json')
  ...
  test('PUT deduplication allowed across repository', async () => {
    await supertest(server)
      .put('/data/cats')
      .send({ name: 'Emmea2' })
      .set('Accept', 'application/json')
